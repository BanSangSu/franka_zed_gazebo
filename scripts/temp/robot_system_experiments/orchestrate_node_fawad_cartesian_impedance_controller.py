...
class SmoothZDescent(py_trees.behaviour.Behaviour):
    ...
    def initialise(self):
        ...
        for s in s_vals:
            ...
            p.position.x = start_pose.position.x #+ dx * s
            p.position.y = start_pose.position.y #+ dy * s
...
def build_pick_sequence():
   ...
   move_until_touch.add_child(
        SmoothZDescent(source_key=BB.TARGET_POSE, z_offset=-0.03, num_steps=60, persistent=True)
    )
