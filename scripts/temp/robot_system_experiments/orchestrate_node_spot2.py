#!/usr/bin/env python3
"""
FIXED Orchestrator - Stack-Aware Logic
Corrected for py_trees 0.7.6 + Dynamic Pathing
"""
...
class Config:
   ...
   # Static heights for picking
   SAFE_HEIGHT_PICK = 0.2
   GRASP_HEIGHT = -0.0145
   
   # Dynamic heights for placing
   PLACE_HEIGHT = 0.005
   STACK_HEIGHT_BUFFER = 0.12 # How much higher than the stack to stay during approach
...
def init_blackboard():
   ...
   bb.set(BB.DYNAMIC_SAFE_Z, Config.SAFE_HEIGHT_PICK)
   return bb
...
class DynamicStackGoal(py_trees.behaviour.Behaviour):
   ...
   def update(self):
        ...
        # Target Z for the center of the next cube in stack
        tz = Config.BASE_Z + (count * Config.CUBE_SIZE) + (Config.CUBE_SIZE / 2)
        # Dynamic safety height: top of current stack + buffer
        dynamic_z = Config.BASE_Z + ((count + 1) * Config.CUBE_SIZE) + Config.STACK_HEIGHT_BUFFER
        
        self.bb.set(BB.TARGET_POSE, make_pose(Config.BASE_X, Config.BASE_Y, tz))
        self.bb.set(BB.DYNAMIC_SAFE_Z, dynamic_z)
        return py_trees.common.Status.SUCCESS
...
class PlanToPose(ActionClient):
   def __init__(self, name, source, offset, use_dynamic_z=False):
       super(PlanToPose, self).__init__(name, "/planning_action", PlanningActionAction)
       self.source = source
       self.offset = offset
       self.use_dynamic_z = use_dynamic_z

   def make_goal(self):
       src = self.bb.get(self.source)
       sel = self.bb.get(BB.SELECTED_CUBE)
       g = PlanningActionGoal(); g.action = ""
       
       # 1. Determine Base Pose
       base = src['pose'] if self.source == BB.SELECTED_CUBE else src
       
       # 2. Apply Offsets (Static vs Dynamic)
       dz = self.bb.get(BB.DYNAMIC_SAFE_Z) if self.use_dynamic_z else base.position.z + self.offset[2]
       g.target_pose = offset_pose(base, self.offset[0], self.offset[1], 0, quat=Config.GRASP_QUAT)
       g.target_pose.position.z = dz

       # 3. Targeted Collision Avoidance (Avoid stack, ignore held cube)
       if sel:
           g.allowed_collision_object = f"cube_{sel['id']}"
       else:
           g.allowed_collision_object = ""
...
def build_tree():
   ...
   pick.add_child(PlanToPose("ApproachPick", BB.SELECTED_CUBE, (0,0,Config.SAFE_HEIGHT_PICK)))
   ...
   pick.add_child(PlanToPose("Lift", BB.SELECTED_CUBE, (0,0,Config.SAFE_HEIGHT_PICK)))
   pick.add_child(Execute())
   
   loop.add_child(Retry("RetryPick", pick, Config.MAX_PICK_RETRIES))
   
   place = py_trees.composites.Sequence("PlaceSequence")
   # 1. Approach from ABOVE the current stack
   place.add_child(PlanToPose("ApproachPlace", BB.TARGET_POSE, (0,0,0), use_dynamic_z=True))
   place.add_child(Execute())
   # 2. Descend to the specific placement Z
   place.add_child(PlanToPose("DescendPlace", BB.TARGET_POSE, (0,0,Config.PLACE_HEIGHT)))
   place.add_child(Execute())
   place.add_child(Detach())
   place.add_child(GripperOpen())
   place.add_child(MarkComplete())
   # 3. Rise back up to the dynamic safety height
   place.add_child(PlanToPose("Clear", BB.TARGET_POSE, (0,0,0), use_dynamic_z=True))
   place.add_child(Execute())
