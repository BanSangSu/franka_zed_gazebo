...
class SamCubeDetector:
    ...
    def create_table(self):
        ...
        # table_pose.pose.position.z = -0.265 # Half table height (5cm total height)
        table_pose.pose.position.z = 0.0025 # Half table height (5cm total height)
        table_pose.pose.orientation.w = 1.0
        
        # Table size: width=0.7m (X), length=0.8m (Y), height=0.05m (Z)
        self.scene.add_box("table", table_pose, size=(0.8, 1.5, 0.01))
    ...
    def update_planning_scene(self, idx, pos, q):
        ...
        if obj_name in self.scene.get_known_object_names():
            rospy.logwarn(f"[Perception] {obj_name} already in scene, updating...")
            self.scene.remove_world_object(obj_name)
            rospy.sleep(0.2)
        
        self.scene.add_box(obj_name, ps, [self.known_cube_size - 0.002]*3)
        rospy.sleep(0.5)  # ← CRITICAL: Wait for scene update
        rospy.loginfo(f"[Perception] ✓ Added {obj_name} at [{pos[0]:.3f}, {pos[1]:.3f}, {pos[2]:.3f}]")
