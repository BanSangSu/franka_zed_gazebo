   ...
   def update(self):
       try:
           ...
           for det in current_detections:
                ...
                for i, old in enumerate(old_cubes):
                ...
                if matched_idx != -1:
                    det['id'] = old_cubes.pop(matched_idx)['id']
                else:
                    det['id'] = next_id
                    next_id += 1
                new_tracked_cubes.append(det)

            # Blackboard update
           self.bb.set(BB.CUBES, new_tracked_cubes)
           self.bb.set(BB.NEXT_ID, next_id)

           rospy.loginfo(f"[{self.name}] Tracked {len(new_tracked_cubes)} cubes. (IDs: {[c['id'] for c in new_tracked_cubes]})")
           return py_trees.common.Status.SUCCESS

       except Exception as e:
           rospy.logerr(f"[{self.name}] Error during perception: {e}")
           return py_trees.common.Status.FAILURE
...
class SelectNextStackGoal(py_trees.behaviour.Behaviour):
   """Select next incomplete goal"""
  
   def __init__(self):
       super(SelectNextStackGoal, self).__init__("SelectNextStackGoal")
       self.bb = py_trees.blackboard.Blackboard()
...
#        return py_trees.common.Status.FAILURE
   def update(self):
        ...
        done_count = len(mapping)  # How many placed
        
        # Find all placed cubes in current perception
        placed_cubes = []
        for goal_idx, cube_id in mapping.items():
            for cube in cubes:
                if cube['id'] == cube_id:
                    placed_cubes.append(cube)
                    break
        
        if not placed_cubes:
            # First cube on table
            target_z = Config.BASE_Z
        else:
            # Stack on highest cube
        ...
        self.bb.set(BB.GOAL_IDX, done_count)
        
        rospy.loginfo(f"Stack goal #{done_count} at Z={target_z:.4f}")
        return py_trees.common.Status.SUCCESS
...
class SelectCube(py_trees.behaviour.Behaviour):
   ...
   def update(self):
       ...
       # Filter out cubes that are already at a completed goal position
       available = []
       safe_dist = Config.CUBE_SIZE * 0.8
      
       for cube in cubes:
           is_placed = False
           cp = cube['pose'].position
           for idx in done:
               gp = goals[idx].position
               dist = np.sqrt((cp.x - gp.x)**2 + (cp.y - gp.y)**2 + (cp.z - gp.z)**2)
               if dist < safe_dist:
                   is_placed = True
                   break
           if not is_placed:
               available.append(cube)
      
       if not available:
           rospy.logwarn(f"[{self.name}] No cubes available (all seem to be placed or none found)")
           return py_trees.common.Status.FAILURE
      
       # Select cube with largest X (furthest from robot usually)
       best = max(available, key=lambda c: c['pose'].position.x)
       self.bb.set(BB.SELECTED_CUBE, best)
      
       rospy.loginfo(f"[{self.name}] Selected cube {best['id']} at ({best['pose'].position.x:.3f}, {best['pose'].position.y:.3f})")
       return py_trees.common.Status.SUCCESS
...
def build_tree(pattern, value):
   ...
   setup.add_child(SelectNextStackGoal())
