...
class PlanningActionServer:
    ...
    def __init__(self):
        ...
        max_velocity = rospy.get_param('~max_velocity', 0.15)
        max_acceleration = rospy.get_param('~max_acceleration', 0.10)
        ...
        self.move_group.set_goal_position_tolerance(rospy.get_param('~goal_position_tolerance', 0.001))
    ...
    def handle_detach(self, goal):
        ...
        if not goal.object_name:
            result.success = False
            result.message = "DETACH action requires object_name"
            return result
        
        if not self.publish_feedback(0.3, f"Detaching {goal.object_name}"):
            return None
        ...
        if goal.object_name not in self.attached_objects:
            rospy.logwarn(f"No metadata for {goal.object_name}, using defaults")
            size = (0.05, 0.05, 0.05)
        else:
            metadata = self.attached_objects[goal.object_name]
            size = metadata['size']
            rospy.loginfo(f"Detaching {goal.object_name} after {(rospy.Time.now() - metadata['attach_time']).to_sec():.1f}s")
        
        # Detach
        self.scene.remove_attached_object(self.ee_link, name=goal.object_name)
        ...
        new_name = goal.object_name.replace("cube_", "placed_cube_")
        ...
        if new_name != goal.object_name:
            self.scene.remove_world_object(goal.object_name)
        
        # Clean up metadata
        if goal.object_name in self.attached_objects:
            del self.attached_objects[goal.object_name]
        
        rospy.loginfo(f"âœ“ Detached {goal.object_name} â†’ {new_name} at [{current_pose.pose.position.x:.3f}, {current_pose.pose.position.y:.3f}, {current_pose.pose.position.z:.3f}]")
        
        if not self.publish_feedback(1.0, "Detach complete"):
            return None
        
        result.success = True
        result.message = f"Object {goal.object_name} detached and added as {new_name}"
        return result
