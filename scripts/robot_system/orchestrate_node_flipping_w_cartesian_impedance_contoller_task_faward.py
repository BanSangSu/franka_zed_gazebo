   ...
   def update(self):
       try:
           ...
           for i in range(resp.num_cubes):
            ...
            is_duplicate = False
            for goal_idx, p_pos in flipped_dict.items():
...
class SwitchController(py_trees.behaviour.Behaviour):
    ...
    def __init__(self, mode):
        ...
        if ns == "/":
            ...
            self.srv_name = '/'+ns+'/controller_manager/switch_controller'
...
class SmoothZDescent(py_trees.behaviour.Behaviour):
    def __init__(self, source_key, z_offset=-0.05, num_steps=40, step_hz=20.0, persistent = False, axis="z", threshold=3.0):
        ...
        if ns == "/":
            ...
            self.pub = rospy.Publisher(
                '/'+ns+'/cartesian_impedance_example_controller/equilibrium_pose',
                PoseStamped, queue_size=10
            )
        self.step_idx = 0
        self.last_time = None
        ...
        if ns == "/":
            ...
            self.sub = rospy.Subscriber(
                '/'+ns+"/franka_state_controller/franka_states", 
                FrankaState, self._state_cb
            )
    ...
    def _state_cb(self, msg):
        ...
        # rospy.loginfo(f"Uncontact, Force spiked by {delta_force:.3f} N (Total: {current_force:.3f} N)")
    ...
    def initialise(self):
        ...
        self.sliding_offset = 0.0003
        self.step_idx = 0
        self.contact_count = 0
        self.last_time = rospy.Time.now().to_sec()
        self.waypoints = []

        self.goal_z_offset = 0.025

        raw = self.bb.get(self.source_key)
        if raw is None:
            rospy.logerr(f"[{self.name}] Source key '{self.source_key}' is None!")
            return
        self.end_pose = raw['pose'] if isinstance(raw, dict) else raw  ##is it necessary??

        
        start_pose = self.move_group.get_current_pose().pose

        dx = self.end_pose.position.x - start_pose.position.x
        dy = self.end_pose.position.y - start_pose.position.y
        dz = (self.end_pose.position.z + self.z_offset) - start_pose.position.z
        ...
        for s in s_vals:
            ...
            rospy.loginfo(f"COUNT [{count}], step: '{s}'")
            count+=1

        rospy.loginfo(
            f"[{self.name}] {len(self.waypoints)} waypoints | "
            f"xyz: ({start_pose.position.x:.3f},{start_pose.position.y:.3f},{start_pose.position.z:.3f})"
            f" → ({self.end_pose.position.x:.3f},{self.end_pose.position.y:.3f},{self.end_pose.position.z + self.z_offset:.3f})"
        )
    ...
    def update(self):
        ...
        rospy.loginfo(f"CURRENT Z: {current_z:.3f}")

        if self.step_idx >= len(self.waypoints):
            rospy.loginfo(f"Finished all waypoints")
            if self.persistent and self.goal_z_offset < current_z:
                ...
                if self.contact_detected:
                    ...
                    p.position.x += self.sliding_offset  * self.contact_count
        ...
        #     rospy.loginfo(f"Working?")
        p = self.waypoints[self.step_idx]
        if self.contact_detected:
            self.contact_count += 1
            rospy.loginfo(f"[{self.name}] Contact Detected!")
            p.position.x += self.sliding_offset * self.contact_count
...
def build_flip_sequence():
   flip = py_trees.composites.Sequence("Flip")
   flip.add_child(GripperOpen())
   flip.add_child(PlanToPose("Approach", BB.SELECTED_CUBE, (Config.FLIPPING_X_OFFSET, 0, Config.SAFE_HEIGHT))) ##can also add the rotation of the cube that will be picked
   flip.add_child(Execute())
   flip.add_child(py_trees.timers.Timer("SwitchWait", duration=1.5))
   flip.add_child(SwitchController("IMPEDANCE"))
   flip.add_child(py_trees.timers.Timer("SwitchWait", duration=1.5))
   ...
   move_until_touch.add_child(
        SmoothZDescent(source_key=BB.SELECTED_CUBE, z_offset=-0.03, num_steps=30, persistent=True, axis="z", threshold=5.0)
    )

   flip.add_child(GripperClose(mode="flipping"))
   flip.add_child(move_until_touch)
   flip.add_child(SmoothZDescent(BB.SELECTED_CUBE, z_offset=Config.SAFE_HEIGHT, num_steps=20))
   flip.add_child(MarkComplete())
   flip.add_child(py_trees.timers.Timer(name="LiftingPause", duration=1.0)) # Small delay for the 'spring' to lift
   flip.add_child(SwitchController("EFFORT"))
   return flip
...
#    recovery.add_child(build_safe_drop_sequence())
   ...
   # Flip with retry
   flip_with_retry = Retry(
       name="RetryFlip",
       child=build_flip_sequence(),
       num_failures=Config.MAX_PICK_RETRIES
   )
   loop.add_child(flip_with_retry)
